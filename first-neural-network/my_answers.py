...
class NeuralNetwork(object):
    ...
    def backpropagation(self, final_outputs, hidden_outputs, X, y, delta_weights_i_h, delta_weights_h_o):
        ...
        # Δwi = η * δ * xi
        delta_weights_h_o += self.lr * np.dot(np.expand_dims(hidden_outputs, -1), np.expand_dims(output_error_term, -1).T)
        ...
        # Δwi = η * δ * xi
        delta_weights_i_h += self.lr * np.dot(X[:, None], np.expand_dims(hidden_error_term, -1).T)
